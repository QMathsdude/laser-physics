...
class FabryPerotApp(tb.Frame):
    ...
    def __init__(self, root):
        ...
        self.vcmd_d = root.register(self.validate_d)
        self.vcmd_labda = root.register(self.validate_labda)
    ...
    def create_sliders(self):
        ...
        slider_d = tb.Scale(self.d_frame,
                            ...
                            to_=10.00,
                            orient=HORIZONTAL,
                            command=self.round_scale_d,)
        entry_d = tb.Entry(self.d_frame,
                           textvariable= self.d,
                           width=8,
                           font=("Liberation Sans", 10),
                           justify="center",
                           validate='key', # Validate on every key press
                           validatecommand=(self.vcmd_d, '%P')) # Pass the future value (%P) to validator
        ...
        slider_labda = tb.Scale(self.labda_frame,
                            ...
                            to_=1000,
                            orient=HORIZONTAL,
                            command=self.round_scale_wavelength,)
        entry_labda = tb.Entry(self.labda_frame,
                           textvariable= self.labda,
                           width=8,
                           font=("Liberation Sans", 10),
                           justify="center",
                           validate='key', # Validate on every key press
                           validatecommand=(self.vcmd_labda, '%P')) # Pass the future value (%P) to validator
    ...
    def validate_R(self, R):
        ...
        if R.isdigit() and int(R) > 0: return False # Also handles cases like "01" by converting to int(1)
        # 3. Check for numeric validity first (or a partial entry)
        try: value = float(R)
        except ValueError:
            # Not a number (e.g., "hello")
            if R.count('.') > 1 or R.startswith('-'): return False
